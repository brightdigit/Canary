...
enum TestData {
  ...
  static var fileAttachment: Attachment {
    Attachment(path: "path/to/file.txt", filename: "file.txt")
  }

  static var dataAttachment: Attachment {
    Attachment(data: "hello".data(using: .utf8)!, filename: "file.txt")
  }
  ...
  static var appState: SentryAppState {
    SentryAppState(releaseName: "1.0.0", osVersion: "14.4.1", isDebugging: false, systemBootTimestamp: timestamp)
  }
  ...
  static func setContext(_ scope: Scope) {
    ...
    static var slowFrameThreshold: Double {
      1 / (Double(maximumFramesPerSecond) - 1.0)
    }
