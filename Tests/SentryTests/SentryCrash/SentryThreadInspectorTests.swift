...
class SentryThreadInspectorTests: XCTestCase {
  ...
  func testStacktraceHasFrames() {
    ...
    XCTAssertTrue(stacktrace?.frames.count ?? 0 > 30, "Not enough stacktrace frames.")
  }
...
private class TestMachineContextWrapper: NSObject, SentryCrashMachineContextWrapper {
  func fillContext(forCurrentThread _: OpaquePointer) {
    // Do nothing
  }

  var threadCount: Int32 = 0
  func getThreadCount(_: OpaquePointer) -> Int32 {
    threadCount
  }

  func getThread(_: OpaquePointer, with _: Int32) -> SentryCrashThread {
    0
  }

  var threadName: String? = ""
  func getThreadName(_: SentryCrashThread, andBuffer buffer: UnsafeMutablePointer<Int8>, andBufLength bufLength: Int32) {
    if threadName != nil {
