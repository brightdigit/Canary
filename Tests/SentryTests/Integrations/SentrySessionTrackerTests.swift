...
class SentrySessionTrackerTests: XCTestCase {
  ...
  private class Fixture {
    ...
    func getSut() -> SessionTracker {
      SessionTracker(options: options, currentDateProvider: currentDateProvider)
    }

    func setNewHubToSDK() {
      let hub = SentryHub(client: client, andScope: nil, andCrashAdapter: sentryCrash, andCurrentDateProvider: currentDateProvider)
  ...
  private func assertNoInitSessionSent() {
    ...
    sessions.sort { first, second in first.started < second.started }
