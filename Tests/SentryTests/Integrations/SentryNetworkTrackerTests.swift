...
class SentryNetworkTrackerTests: XCTestCase {
  ...
  private class Fixture {
    ...
    var mutableUrlRequest: URLRequest {
      URLRequest(url: SentryNetworkTrackerTests.testURL)
    }
  }
  ...
  private func startTransaction() -> Span {
    SentrySDK.startTransaction(name: SentryNetworkTrackerTests.transactionName, operation: SentryNetworkTrackerTests.transactionOperation, bindToScope: true)
  }

  private func createResponse(code: Int) -> URLResponse {
    HTTPURLResponse(url: SentryNetworkTrackerTests.testURL, statusCode: code, httpVersion: "1.1", headerFields: nil)!
  }
