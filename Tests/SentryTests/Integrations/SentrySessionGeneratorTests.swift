...
class SentrySessionGeneratorTests: XCTestCase {
  ...
  override func setUp() {
    ...
    options.integrations = Options.defaultIntegrations().filter { name -> Bool in
      name != "SentryAutoSessionTrackingIntegration"
    }
  ...
  private func startSdk() {
    ...
    let hub = SentryHub(client: client, andScope: nil, andCrashAdapter: sentryCrash, andCurrentDateProvider: DefaultCurrentDateProvider.sharedInstance())
