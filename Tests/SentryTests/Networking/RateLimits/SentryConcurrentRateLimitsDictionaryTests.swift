...
class SentryConcurrentRateLimitsDictionaryTests: XCTestCase {
  ...
  func testTwoRateLimit() {
    let dateA = currentDateProvider.date()
    ...
    XCTAssertEqual(dateA, sut.getRateLimit(for: SentryRateLimitCategory.default))
    XCTAssertEqual(dateB, sut.getRateLimit(for: SentryRateLimitCategory.error))
  }
  ...
  func testOverridingRateLimit() {
    ...
    XCTAssertEqual(dateA, sut.getRateLimit(for: SentryRateLimitCategory.attachment))

    sut.addRateLimit(SentryRateLimitCategory.attachment, validUntil: dateB)
    XCTAssertEqual(dateB, sut.getRateLimit(for: SentryRateLimitCategory.attachment))
  }
  ...
  func testConcurrentReadWrite() {
    ...
    for i in 0 ... 10 {
      let date = currentDateProvider.date().addingTimeInterval(TimeInterval(i))
