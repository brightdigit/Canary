...
public class Dynamic: CustomDebugStringConvertible, Loggable {
  ...
  public func dynamicallyCall(withKeywordArguments pairs: KeyValuePairs<String, Any?>) -> Dynamic {
    ...
    callMethod(selector, with: pairs.map(\.value))
    return self
  }
...
public extension Dynamic {
  ...
  var asObject: NSObject? { asAnyObject as? NSObject }
  var asArray: NSArray? { asAnyObject as? NSArray }
  var asDictionary: NSDictionary? { asAnyObject as? NSDictionary }
  var asString: String? { asAnyObject?.description }
  var asInt8: Int8? { unwrap() }
  var asUInt8: UInt8? { unwrap() }
  var asInt16: Int16? { unwrap() }
  var asUInt16: UInt16? { unwrap() }
  var asInt32: Int32? { unwrap() }
  var asUInt32: UInt32? { unwrap() }
  var asInt64: Int64? { unwrap() }
  var asUInt64: UInt64? { unwrap() }
  var asFloat: Float? { unwrap() }
  var asDouble: Double? { unwrap() }
  var asBool: Bool? { unwrap() }
  var asInt: Int? { unwrap() }
  var asUInt: UInt? { unwrap() }
  var asSelector: Selector? { unwrap() }

  func asInferred<T>() -> T? { unwrap() }
  ...
  public extension Dynamic {
    var asCGPoint: CGPoint? { unwrap() }
    var asCGVector: CGVector? { unwrap() }
    var asCGSize: CGSize? { unwrap() }
    var asCGRect: CGRect? { unwrap() }
    var asCGAffineTransform: CGAffineTransform? { unwrap() }
    var asUIEdgeInsets: UIEdgeInsets? { unwrap() }
    var asUIOffset: UIOffset? { unwrap() }

    #if !os(watchOS)
      var asCATransform3D: CATransform3D? { unwrap() }
    #endif
  }
#endif
