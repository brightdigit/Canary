...
class Invocation: Loggable {
  ...
  private func initialize() throws {
    ...
    numberOfArguments = methodSignature.value(forKeyPath: "numberOfArguments") as? Int ?? 0
    log("NumberOfArguments:", numberOfArguments)

    /// `methodReturnLength = methodSignature.methodReturnLength`
    returnLength = methodSignature.value(forKeyPath: "methodReturnLength") as? Int ?? 0
    ...
    log("ReturnType:", returnTypeString ?? "?")
  ...
  func getReturnValue<T>(result: inout T) {
    ...
    if NSStringFromSelector(selector) == "alloc" {
...
public enum InvocationError: CustomNSError {
  ...
  public var errorUserInfo: [String: Any] {
    var message: String
    switch self {
    case let .unrecognizedSelector(classType, selector):
