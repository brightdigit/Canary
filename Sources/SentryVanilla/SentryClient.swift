...
public struct SentryClient: ISentryClient {
  ...
  public func capture(event: SentryEvent, scope: Scope? = nil) {
    ...
    if let beforeSend = options.beforeSend {
  ...
  private func send(event: SentryEvent) {
    ...
    let auth = "Sentry sentry_version=6,sentry_client=sentry.swift/0.0.1,sentry_key=\(dsn.publicKey),sentry_timestamp=\(timestamp)"
    print(auth)
